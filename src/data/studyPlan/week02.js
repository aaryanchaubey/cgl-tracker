// Week 2 — May 2 (Thu) to May 8 (Wed) — Phase 1: Pre-Exam Foundation
...
export const DAYS = [
  {
    ...
    title:'Day 8 — Spelling Mastery + Mixture & Alligation',
    promptBlock:`🎯 OPERATIVE BRIEFING — DAY 8
...
Missing Number (matrix/figure problems) + Number Sequences. Missing Number requires you to identify if the pattern is row-wise, column-wise, diagonal, or ring-based. Always check all four before giving up.`,
    tasks: [
      WD_ENG('d8',[
        st('d8-e1','Spelling Corrections — 40 Common SSC Words','Learn correct spelling of most-misspelled words: Accommodation, Necessary, Committee, Occurrence, Maintenance, Bureaucracy, Supersede, Millennium, Rhythm, Conscientious, Conscience, Questionnaire, Separate, Definitely, Acquaintance.','Rule: "i before e except after c" has exceptions. Memorize exceptions: weird, seize, either, neither, leisure.',20),
        st('d8-e2','Confusable Word Pairs — 20 Pairs','Study pairs: Effect/Affect, Principle/Principal, Stationary/Stationery, Complement/Compliment, Eminent/Imminent, Advice/Advise, Fewer/Less, Farther/Further. Write each in a sentence.','SSC loves confusable words in error-spotting AND fill-in-the-blanks. Two exam applications.',25),
      ]),
      WD_GA('d8',[
        st('d8-ga1','Current Affairs — Science & Technology','Focus: Recent ISRO missions, DRDO achievements, IT ministry regulations, major scientific discoveries, digital India updates.','Tech CA: Note the full name of abbreviations — ISRO, DRDO, DPIIT etc. Examiners test full forms.',20),
        st('d8-ga2','Science — Physics: Laws & Principles','Newton\'s 3 Laws, Laws of Thermodynamics (1st=energy conservation, 2nd=entropy), Ohm\'s Law, Faraday\'s Laws. Key units: Force=Newton, Energy=Joule, Power=Watt.','For SSC Physics: Focus on applications and unit recognition. Less theory, more applied MCQs.',25),
      ]),
      WD_QR('d8',[
        st('d8-qr1','40 PYQs — Compound Interest + Mixture & Alligation','CI: Multiplier method. A=P×(1+r/100)^n. For 2 years: CI-SI = P(r/100)². Alligation: Crosshatch diagram. Draw (cheaper price — mean price) : (mean price — costlier price) = ratio of mixing.','Alligation example: Milk at ₹20 and ₹30, mean ₹24 → ratio = (30-24):(24-20) = 6:4 = 3:2.',35),
        st('d8-qr2','Missing Number + Number Sequences — 50 Qs','Missing Number: Check row sums, column products, diagonal patterns, ring patterns. Number Sequences: Prime gaps, Fibonacci variants, square/cube sequences, alternating patterns.','If the missing number is in a 3×3 grid: try (row1+row2)/2, or row1×row2-something. Systematic testing.',25),
      ]),
    ],
  },
  {
    ...
    weekType:'weekday', totalHours:2.5, hasQuiz:true,
    title:'Day 9 — Phrasal Verbs + Boats & Streams',
    promptBlock:`🎯 OPERATIVE BRIEFING — DAY 9
...
Alpha-Numeric Series — SSC loves mixing letters and numbers. Key: code each letter as its position (A=1, Z=26) or reverse position (A=26). Then check if the number pattern matches letter positions. Analogy review — missed analogy types from Week 1.`,
    tasks: [
      ...
      WD_QR('d9',[
        ...
        st('d9-qr2','Alpha-Numeric Series + Analogy Revision — 50 Qs','Alpha-numeric: Decode letters by position, then find numerical pattern. Analogy: Identify the relationship type (synonym, antonym, part-whole, tool-use, cause-effect, worker-product).','Never assume a single analogy type. Always check: is it 1:2 ratio? Is it A:A² ? Is it Country:Capital?',25),
      ]),
    ],
  },
  {
    ...
    title:'Day 10 — Mensuration 2D Mastery + Tenses Deep Dive',
    promptBlock:`🎯 OPERATIVE BRIEFING — DAY 10 (WEEKEND)
...
MOCK INTENSITY: Full mock + 2 sectionals. Compare this week's mock percentile with Week 1. Are you moving up?`,
    tasks: [
      ...
      WE_MT('d10',[
        ...
        st('d10-mt2','Sectional: Quant 30-min + English 30-min','Back-to-back sectionals. In Quant: apply Mensuration formulas from today. In English: apply Tense identification to error-spotting questions.','Immediate application of learned concepts is the best test of retention.',60),
      ]),
      WE_REV('d10',[
        st('d10-rev1','Error Category Count — Week 2 Trend','Total errors across Week 1 and Week 2 mocks. Which category (Conceptual/Calculation/Optimal Path) is dominant? This guides next week\'s strategy adjustment.','If Conceptual errors dominate: more deep study time on weekends. If Calculation errors: more timed drills on weekdays.',30),
        st('d10-rev2','Formula Book Update — Mensuration Edition','Add all 2D and 3D formulas studied today. Use a visual layout — each shape as a small diagram with formulas annotated.','Visual formula cards are recalled 3× faster during exams than text lists.',30),
      ]),
    ],
  },
  {
    ...
    weekType:'weekend', totalHours:8, hasQuiz:true,
    title:'Day 11 — Mensuration 3D Complete + Reading Stamina',
...
MOCK: Same intensity. Track your overall percentile trend. Week 2 should show 3-5 percentile points improvement over Week 1 baseline.`,
    tasks: [
      ...
      WE_MT('d11',[
        ...
        st('d11-mt2','Sectional: Reasoning 30-min + Quant 30-min','Reasoning: Target 15-17 minutes for 25 questions (banking 3 min for review). Quant: Apply Mensuration formulas from today.','If Reasoning target of 17 minutes is met consistently, you have banked 3 extra minutes for Quant.',60),
      ]),
      WE_REV('d11',[
        st('d11-rev1','RC Speed Measurement','Calculate today\'s WPM from the stamina drill. Compare with target of 120 WPM. Record in your tracker.','WPM growth is steady — 5 WPM per week with consistent practice. Don\'t expect overnight jumps.',30),
        st('d11-rev2','Mensuration Self-Test','Without notes, write all 3D shape formulas from memory. Score yourself. Any gaps = 10-minute focused re-review.','Self-testing is 3× more effective than re-reading for formula retention.',30),
      ]),
    ],
  },
  {
