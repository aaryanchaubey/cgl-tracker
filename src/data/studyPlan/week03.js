// Week 3 — May 9 (Sat) to May 15 (Fri) — Phase 1 | Days 15-21 | ALL HAVE QUIZ
...
export const DAYS=[
  ...
  // ── Day 17 — Mon May 11 — WEEKDAY (2.5 hours) ────────────────────
  {
    ...
    tasks:[
      ...
      q('d17',[
        ...
        st('d17-qr2','Pattern Completion + Figure Series — 40 Qs','Identify: rotation angle, number of elements added/removed, shading patterns, size changes. Apply rule to predict missing figure.','Systematic check: (1) Rotation? (2) Reflection? (3) Addition/subtraction of elements? (4) Shading pattern? In that order.',25),
      ]),
    ],
  },

  // ── Day 18 — Tue May 12 — WEEKDAY (2.5 hours) ────────────────────
  {
    id:'2026-05-12',dayNumber:18,date:'2026-05-12',phase:1,tier:1,weekNumber:3,
    weekType:'weekday',totalHours:2.5,hasQuiz:true,
    title:'Day 18 — Sentence Improvement + Height & Distance',
    ...
    tasks:[
      e('d18',[
        st('d18-e1','20 Sentence Improvement PYQs','Strategy for each: (1) Is the original correct? (2) If not, what grammar rule does it violate? (3) Which option correctly applies the rule?','Never choose an option just because it "sounds better." Every change must be justified by a rule.',25),
        st('d18-e2','Preposition Usage — 10 Key Rules + 10 PYQs','Key prepositions: "different from" (not "different than"), "inferior to" (not "inferior than"), "listen to" (not "listen"), "die of" disease (not "die from"), "abide by" (not "abide with").','Preposition errors are the second most common error type in SSC after SVA. Learn fixed prepositions with verbs/adjectives.',20),
      ]),
      g('d18',[
        st('d18-ga1','Current Affairs — Environment & Climate','Focus: COP summits, India\'s climate commitments (Net Zero 2070), major environmental policies, endangered species news, national parks/wildlife sanctuaries in news.','Climate CA: India\'s renewable energy targets (500 GW by 2030), solar capacity milestones. Know the numbers.',20),
        st('d18-ga2','Economy — GDP, Inflation & Key Indicators','India\'s GDP growth rate, CPI/WPI inflation, unemployment rate, fiscal deficit as % of GDP, foreign exchange reserves. Latest figures from Economic Survey.','These numbers appear directly in MCQs. Must know the most recent official figures.',25),
      ]),
      q('d18',[
        st('d18-qr1','30 PYQs — Height & Distance','Every question: draw diagram first. Label: height (h), base distance (d), angle (θ). Apply: tanθ=h/d, sinθ=h/hypotenuse, cosθ=d/hypotenuse. Two-angle problems: set up two equations, solve.','Common setup: Tower AB, observer at C. Angle of elevation = θ. tanθ = AB/BC → BC = AB/tanθ. Height asked = use cotθ or tan(complementary angle).',35),
        st('d18-qr2','Number + Alphabet Sequences — 40 Qs','Number sequences: arithmetic (fixed difference), geometric (fixed ratio), prime-based, square/cube-based, Fibonacci-type. Alphabet sequences: +n positions, alternating, skip patterns.','First identify if it\'s a single sequence or split into even/odd terms. Many SSC series alternate between two independent sequences.',25),
      ]),
    ],
  },

  // ── Day 19 — Wed May 13 — WEEKDAY (2.5 hours) — CORRECT ──────────
  {
  ...
  // ── Day 20 — Thu May 14 — WEEKDAY (2.5 hours) — CORRECT ──────────
  {
  ...
  // ── Day 21 — Fri May 15 — WEEKDAY (2.5 hours) — CORRECT ──────────
  {
